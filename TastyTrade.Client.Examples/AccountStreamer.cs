...
namespace TastyTrade.Client.Examples
{
    public class AccountStreamer
    {
        ...
        public static void SetupSerializationAttributeStringMaps() {
            ...
            var updateTypeMemberInfo = streamingAccountOrderUpdateType.GetMember(_StreamingAccountOrderUpdate_Type).First();
            ...
            var actionMessageActionTypeMemberInfo = actionMessageActionType.GetMember(_SubscriptionActionMessageResponse_Action).First();
        ...
        public static async Task Run()
        {
            ...
            using (var ws = new ClientWebSocket())
            {
                ...
                System.Threading.Timer heartbeatTimer = new System.Threading.Timer((s) => {
                ...
                Console.WriteLine($"{nameof(AccountStreamer)} runloop exited: {DateTime.UtcNow.ToString("yyyyMMddHHmmss")}");
            }
        ...
        private static void HandleMessage(byte[] buffer, int count)
        {
            ...
            {
                var connectMsgBody = JsonSerializer.Deserialize<SubscriptionActionMessageResponse<string>>(msgBody);
                _accountUpdates.ConnectionStatus = AccountDataUpdatesConnectionStatus.Open;
                _accountUpdates.ConnectionOpened = DateTime.UtcNow;
                Console.WriteLine($"connect confirmation received: {DateTime.UtcNow.ToString("yyyyMMddHHmmss")}");
            }
